...
import { collectServices, extractFields, scalarName, isArrayType, isRecordType, isModelType, isUnionType, arrayElementType, recordElementType, toSnakeCase, dottedPathToPascalCase, checkAndReportReservedKeywords, safeFieldName, } from "@specodec/typespec-emitter-core";
function typeToScala(type) {
    if (isArrayType(type))
        return `Seq[${typeToScala(arrayElementType(type))}]`;
    if (isRecordType(type))
        return `Map[String, ${typeToScala(recordElementType(type))}]`;
...
function generateFieldRead(L, f, varName, indent) {
    if (isArrayType(f.type)) {
        ...
        const tmp = `tmp${scalaFieldReadCounter++}`;
...
function readExpr(type, r, optional) {
    ...
    if (type.kind === "Model" && type.name) {
        const decodeCall = `${type.name}Codec.decode.apply(${r})`;
        if (optional)
            return `if (${r}.isNull()) { ${r}.readNull(); None } else { Some(${decodeCall}) }`;
        return decodeCall;
    }
    if (type.kind === "Enum") {
...
function generateModelCode(m, _pkg) {
    ...
    const scalaField = (f) => safeFieldName("scala", toSnakeCase(f.name));
    const lines = [];
...
function generateUnionCode(u, L) {
    ...
    const variantName = (v) => unionName + v.name.charAt(0).toUpperCase() + v.name.slice(1);
